...
Status WriteBatch::IterateWithStartSequence(Handler *handler) const {
  if (rep_.size() < WriteBatchInternal::kHeader) {
    return Status::Corruption("malformed WriteBatch (too small)");
  }

  return WriteBatchInternal::IterateWithStartSequence(this, handler, WriteBatchInternal::kHeader,
                                     rep_.size());
}
...
// optimize for non-recovery mode
    ...
    if (!moptions->inplace_update_support) {
      // KeyFeatures* key_features = new ;
      // key_features.sequence = sequence_;
      // key_features.time_stamp = db_->GetEnv()->NowMicros();
      // key_features.key_range_id = 0;
      // // key_features.write_rate_mb_per_sec = db_->;
      // key_features.adjaccent_key_range_write_rate = 0;

      // uint64_t lifetime_label=0;
      // ret_status = db_->PredictLifetimeLabel(key, key_features, &lifetime_label);
      // if(!ret_status.ok()) {
      //   return ret_status;
      // }
